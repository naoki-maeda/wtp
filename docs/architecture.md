...
Shell scripts generated by `wtp hook <shell>` or `wtp shell-init <shell>` wrap `wtp cd` and perform directory changes in the current shell (bash, zsh, fish). They also support auto-switching after successful `wtp add` in interactive TTY sessions.
